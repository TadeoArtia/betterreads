...
export const getDescription = (searchResults: any) => {
	let desc = ""
	if (typeof searchResults.description === 'string') desc = searchResults.description
	if (typeof searchResults.description?.value === 'string') desc = searchResults.description.value
	...
	console.log(desc)
	let parsed = remark().use(html).processSync(desc).
		toString()
